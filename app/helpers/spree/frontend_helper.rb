module Spree
  module FrontendHelper
    ...
    def spree_breadcrumbs(taxon, _separator = '', product = nil)
      ...
      if taxon
        ...
        if product
          crumbs << content_tag(:li, content_tag(
            :span, content_tag(
              :span, raw(product.name), itemprop: 'name'
    ...
    def checkout_progress(numbers: false)
      ...
      items = states.each_with_index.map do |state, i|
        ...
        if state_index < current_index
          ...
          link_content << text
          content_tag('li', content_tag('a', link_content, class: "d-flex flex-column align-items-center #{if state == @order.state
                                                                                                             'active'
                                                                                                           end}"), class: css_classes.join(' '))
        end
      end
    ...
    def permitted_product_params
      product_filters = available_option_types.is_a?(Hash) ? available_option_types.keys.map(&:name) : available_option_types.map(&:name)
    ...
    def carousel_image_source_set(image)
      ...
      widths.each do |key, value|
        file = s3_persisted_url(image.url("plp_and_carousel_#{key}"), true)
    ...
    def icon(name:, width:, height:, classes: '')
    ...
    def slider_cache_key(slides = @slides)
      @slider_cache_key ||= Digest::MD5.hexdigest(@slides.pluck(:updated_at).max.to_s)
    end
