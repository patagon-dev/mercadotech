module Spree
  class LineItem < Spree::Base
    ...
    before_validation :custom_copy_price
    ...
    # Uses the alias of copy price with new def
    # Fix for overriding the volume pricing: copy price method
    def custom_copy_price
      if variant
        update_price if price.nil?
        self.cost_price = variant.cost_price if cost_price.nil?
        self.currency = variant.currency if currency.nil?

        set_volumne_or_custom_price
      end
    end
    ...
    # Responsible for setting volume or custom price
    def set_volumne_or_custom_price
      if changed? && (changes.keys.include?('quantity') || changes.keys.include?('price'))
        vprice = variant.volume_price(quantity, order.user)
        price_to_compare = custom_price.present? ? custom_price : variant.price

        if price.present? && vprice <= price_to_compare
          vprice = custom_price if custom_price.present? && custom_price > variant.price
          self.price = vprice and return
        end
      end

      self.price = variant.price if price.nil?
      self.price = custom_price if custom_price.present?
    end
    ...
    def update_bypass_stock!(value = false)
      variant.update_column(:by_pass_stock, value)
    end
    ...
    def update_inventory
      verify_order_inventory if (saved_changes? || target_shipment.present?) && order.has_checkout_step?('delivery')
    end
    ...
    def ensure_proper_currency
      errors.add(:currency, :must_match_order_currency) unless currency == order.currency
    end
  end
end
