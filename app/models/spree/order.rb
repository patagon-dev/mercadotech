...
module Spree
  class Order < Spree::Base
    ...
    SHIPMENT_STATES = %w[backorder canceled partial pending ready shipped pickup packed waiting attention].freeze
    ...
    insert_checkout_step :freeze, after: :complete, if: proc { |order|
                                                          (order.has_valid_payment? && order.completed_at.blank?) || order.state == 'freeze'
                                                        }
    insert_checkout_step :webpay_ws_mall, after: :payment, if: proc { |order|
                                                                 order.has_webpay_ws_mall_payment_method? || order.state == Spree::Gateway::WebpayWsMall.STATE
                                                               }
    insert_checkout_step :oneclick_mall, after: :payment, if: proc { |order|
                                                                order.has_oneclick_mall_payment_method? or order.state == Gateway::WebpayOneclickMall.STATE
                                                              }
    remove_checkout_step :confirm

    fsm = state_machines[:state]
    fsm.after_transition from: fsm.states.map(&:name), to: [:complete], do: :restore_bypass_stock_to_default, if: proc { |order|
                                                                                                                    order.has_webpay_ws_mall_payment_method? || order.has_oneclick_mall_payment_method?
                                                                                                                  }

    self.whitelisted_ransackable_associations = %w[shipments user promotions bill_address ship_address line_items store
                                                   invoices]
    self.whitelisted_ransackable_attributes = %w[completed_at email number state payment_state shipment_state total
                                                 considered_risky channel reference_order_numbers]

    attr_reader :coupon_code
    attr_accessor :temporary_address, :temporary_credit_card, :use_billing
    ...
    with_options dependent: :destroy do
      ...
      has_many :micro_compra_purchase_orders, class_name: 'Spree::MicroCompraPurchaseOrder'
      has_many :customer_purchase_orders, class_name: 'Spree::CustomerPurchaseOrder'
    ...
    scope :reverse_chronological, lambda {
                                    order(Arel.sql('spree_orders.completed_at IS NULL'), completed_at: :desc, created_at: :desc)
                                  }
    ...
    def returnable_period
      Time.now < (completed_at + 60.days)
    end

    SHIPPED_STATES = %w[shipped partial].freeze
    ...
    def allow_cancel?
      ...
      shipment_state.nil? || %w[ready backorder pending pickup packed].include?(shipment_state)
    end
    ...
    def find_line_item_by_variant(variant, options = {})
      line_items.detect do |line_item|
        line_item.variant_id == variant.id &&
          Spree::Dependencies.cart_compare_line_items_service.constantize.new.call(order: self, line_item: line_item,
    ...
    def outstanding_balance
      ...
      elsif shipments.invalid.any?
        shipments.valid.map(&:final_price_with_items).sum - payment_total
      else
        total - payment_total
      end
    end
    ...
    def ensure_line_items_are_in_stock
      insufficient_stock_line_items = insufficient_stock_lines

      if (state == Spree::Gateway::WebpayWsMall.STATE || state == Gateway::WebpayOneclickMall.STATE) && insufficient_stock_line_items.present?
        insufficient_stock_line_items.map { |li| li.update_bypass_stock!(true) }
        true
      elsif insufficient_stock_line_items.present?
        restart_checkout_flow
    ...
    def state_changed(name)
      ...
      if persisted?
        ...
        log_state_changes(state_name: name, old_state: old_state, new_state: new_state) unless old_state == new_state
      end
    end
    ...
    def display_vendor_specific_total(vendor)
      Spree::Money.new(vendor.total_amount(self), { currency: currency })
    end

    def display_vendor_specific_ship_total(vendor)
      Spree::Money.new(vendor.shipment_total(self), { currency: currency })
    end

    def vendor_included_tax_total(vendor)
      line_items.for_vendor(vendor).sum(:included_tax_total).round + shipments.for_vendor(vendor).sum(:included_tax_total).round
    end

    def vendor_additional_tax_total(vendor)
      line_items.for_vendor(vendor).sum(:additional_tax_total).round + shipments.for_vendor(vendor).sum(:additional_tax_total).round
    end
    ...
    def vendor_items_weight(vendor)
      line_items.for_vendor(vendor).map(&:variant).sum(&:weight).to_f
    end
    ...
    def webpay_ws_mall_payment_completed?
      if payments.completed.from_webpay_ws_mall.any?
        true
      else
        false
      end
    ...
    def webpay_ws_mall_client_name
      if ship_address
    ...
    #
    # Return TrueClass||FalseClass instance
    def has_webpay_ws_mall_payment_method?
      payments.valid.from_webpay_ws_mall.any?
    end
    ...
    def webpay_ws_mall_payment_method
      has_webpay_ws_mall_payment_method? ? payments.valid.from_webpay_ws_mall.order(:id).last.payment_method : nil
    end

    # Entrega en valor total en un formato compatible con el estandar de Webpay
    #
    # Return String instance
    def webpay_ws_mall_amount
      # TODO: - Ver que pasa cuando hay decimales
      total.round
    end

    # Indica si la orden tiene un pago con código de respuesta 0
    #
    # Return TrueClass||FalseClass instance
    def has_valid_payment?
      payments.webpay_accepted.any?
    end

    def freeze!
      update_attribute :state, 'freeze'
      touch :completed_at
    end

    def get_vendors
      Spree::Vendor.where(id: line_items.joins(:product).pluck('spree_products.vendor_id').uniq)
    end

    def allow_shipment_state_change?
      %w[waiting ready pickup packed attention].include?(shipment_state)
    end

    def get_address_with_type(address_type)
    ...
    def line_items_per_vendor(user)
      user.vendor? ? line_items.joins(:product).where('spree_products.vendor_id =?', user.vendor_ids) : line_items
    end

    def payment_per_vendor(user)
      user.vendor? ? payments.where('vendor_id =?', user.vendor_ids) : payments
    end

    def shipments_per_vendor(user)
      if user.vendor?
        shipments.joins(:stock_location).where('spree_stock_locations.vendor_id =?',
                                               user.vendor_ids)
      else
        shipments
      end
    ...
    def oneclick_mall_payment_completed?
      payments.completed.from_oneclick_mall.any? ? true : false
    end
    ...
    def execute_stock_api
      all_responses = []
      prod_names = []

      shipments.each do |shipment|
        stock_location = shipment.stock_location
        next unless stock_location.enable_stock_api && shipment.line_items.present?

        shipment.line_items.each do |line_item|
          response = StockApi::ApiService.new(line_item.id, stock_location.id).execute
          all_responses << response[:success]
          prod_names << response[:product_name] if response[:success] == false
        end
        return true if all_responses.empty? || !(all_responses.include? false)

        if all_responses.include? false
          errors.add(:base,
                     Spree.t(:product_not_in_stock,
                             prod_name: prod_names.uniq.to_s)) && (return false)
        end
      end
    end
    ...
    def ensure_line_items_present
      errors.add(:base, Spree.t(:there_are_no_items_for_this_order)) && (return false) unless line_items.present?
    end
    ...
    def after_cancel
      shipments.valid.each(&:cancel!)
      payments.completed.from_external.each(&:cancel_payment!)
    ...
    def collect_payment_methods(store = nil)
      PaymentMethod.available_on_front_end.select do |pm|
        pm.available_for_order?(self) && pm.available_for_store?(store)
      end
    end
    ...
    def restore_bypass_stock_to_default
      line_items.map(&:update_bypass_stock!)
    end
