    ...
    module Processing
      ...
      def process!
        if payment_method&.auto_capture?
          ...
          authorize!
          refund_payment! if self.amount < 0 && order.payment_state == 'credit_owed' && check_payment_method
        end
      end
      ...
      def purchase!
        if payment_method.type == 'Spree::Gateway::MicroCompra'
          process_purchase
        else
      ...
      def cancel_payment!
        response = payment_method.cancel(number)
        handle_response(response, response.params['state'].to_sym, :failure)
      end
      ...
      def refund_payment!
        response = payment_method.refund_payment(amount, response, gateway_options)
        handle_response(response, :complete, :failure)
      end
      ...
      def check_payment_method
        Spree::PaymentMethod::REFUND_PAYMENT_METHODS.include? self.payment_method.class
      end
    end
  end
end
