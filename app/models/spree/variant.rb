module Spree
  class Variant < Spree::Base
    ...
    delegate :description, :slug, :available_on, :shipping_category_id,
    ...
    before_save :prepend_vendor_id_in_sku, if: :sku_changed?
    after_create :create_stock_items
    after_create :set_master_out_of_stock, unless: :is_master?

    after_touch :clear_in_stock_cache
    after_update :update_stock_items, if: :saved_change_to_vendor_id?
    ...
    def update_stock_items
      vendor_stock_locations = vendor&.stock_locations
      if vendor_stock_locations.present?
        stock_items.where.not(stock_location_id: vendor_stock_locations.ids).destroy_all
        current_location_ids = stock_items.pluck(:stock_location_id)

        vendor_stock_locations.each do |stock_location|
          next if current_location_ids.include?(stock_location.id)

          stock_location.propagate_variant(self)
        end
      else
        stock_items.destroy_all
      end
    end

    def prepend_vendor_id_in_sku
      sku.prepend("#{product.vendor_id}_")
    end
