        ...
        class << self
          def authorize(username:, tbk_user:, parent_buy_order:, details:,
            ...
            details_array = details.map do |det|
              hash = det.each_with_object({}) do |(k, v), acc|
                acc[k.to_s] = v
              end
            ...
            if resp.is_a? Net::HTTPSuccess
              return ::Transbank::Webpay::Oneclick::MallTransactionAuthorizeResponse.new(body)
            end
          ...
          def status(buy_order:, options: nil)
            ...
            body = JSON.parse(resp.body)
            return ::Transbank::Webpay::Oneclick::MallTransactionStatusResponse.new(body) if resp.is_a? Net::HTTPSuccess
          ...
          def refund(buy_order:, child_commerce_code:, child_buy_order:, amount:,
            ...
            resp = http_post(uri_string: url, body: body, headers: headers, camel_case_keys: false)
            body = JSON.parse(resp.body)
            return ::Transbank::Webpay::Oneclick::MallTransactionRefundResponse.new(body) if resp.is_a? Net::HTTPSuccess
