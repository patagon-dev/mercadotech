...
module DeviseSamlAuthenticatable
  module SamlConfig
    ...
    def file_based_config
      ...
      host = request.host
      Rails.logger.warn "HOST IS: #{host}"

      if File.exist?(idp_config_path)
        @file_based_config ||= OneLogin::RubySaml::Settings.new(YAML.load(File.read(idp_config_path))[Rails.env][host])
      end
    end
    ...
    def adapter_based_config(idp_entity_id)
      ...
      idp_settings_adapter.settings(idp_entity_id).each do |k, v|
        ...
        config.send(acc, v) if config.respond_to? acc
      end
      config
    end
